      ...
      {items.map((item, index) => (
        <DropdownItem key={`${item.href}-${index}`} className="p-0 rounded bg-slate-50">
          {item.children ? (
            ...
            <NextLink passHref href={item.href ? item.href : "/"}>
              <Button className="text-left bg-slate-50 text-black w-full rounded" color="primary" variant="light">
                {item.label}
              </Button>
            </NextLink>
...
export default function Navbarr() {
  const data: DropdownItemType[] = [
    { label: "Trung tâm Báo chí TP Hồ Chí Minh" }
  ];
  ...
  const dropdownData: Record<string, DropdownItemType[]> = {
    "Giới thiệu": [
      ...
      },
      {
        ...
        children: [
          { label: "Văn Kiện Đại Hội" },
          { label: "Ban Chấp Hành Các Thời Kỳ" },
    ...
    "Tin tức": [
      { label: "Đời sống lao động" },
      { label: "Thông tin hoạt động" },
      { label: "Truyền hình công nhân công đoàn" },
      { label: "Tin công đoàn (Báo Người Lao Động)" },
      { label: "Tuyên truyền" },
      { label: "Công khai tài chính" },
    ],
    "Điều hành": [
  ...
  // Extracted children
  const extractedChildren = extractChildren(dropdownData);
